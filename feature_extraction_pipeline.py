...
def process(image, time_stamp, camera_id):
    ...
    for car in detections:
        ...
        car_id = database_connection.query(
            f'SELECT car_id FROM db.data WHERE license_plate_number = "{license_number}"'
        )
        if car_id is not None:
            database_connection["db.data"].insert(
                car_id, car_type, car_color, license_number, time_stamp
            )
            database_connection["db.data"].insert(camera_id, time_stamp, car.id),

        else:
            database_connection["db.data"].insert(
                car_type, car_color, license_number, time_stamp
            )
            database_connection["db.data"].insert(camera_id, time_stamp, car.id),
